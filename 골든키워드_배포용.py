...
class KeywordApp(ctk.CTk):
    ...
    def _get_keyword_stats(self, access_key: str, secret_key: str, customer_id: str, hint_keywords: list) -> dict:
        ...
        try:
            self.log_message(f"[Debug Gold] API 요청 URL: {url}")
            self.log_message(f"[Debug Gold] API 요청 Headers: {headers}")
            self.log_message(f"[Debug Gold] API 요청 Params: {params}")
...
class BlogAnalyzerApp(ctk.CTk):
    CONFIG_FILE = "config_blog_analyzer.json" # 별도의 설정 파일 사용
    MAX_KEYWORDS = 10000 # KeywordApp과 동일하게 유지

    def __init__(self):
        print("BlogAnalyzerApp 초기화 시작...")
        super().__init__()
        ctk.set_appearance_mode("dark")
        ctk.set_default_color_theme("blue")

        self.title("네이버 블로그 포스팅 분석기")
        self.geometry("1024x850")
        self.resizable(True, True)

        self.stop_event = threading.Event()
        self.current_thread = None
        self.all_analysis_data = [] # 분석 결과를 저장할 리스트
        self.last_saved_file = None
        self.log_queue = queue.Queue()

        self.api_entries = {} # KeywordApp과 동일하게 API 입력 필드 관리
        self.blog_id_entry = None # 새로운 Blog ID 입력 필드
        self.status_var = ctk.StringVar(value="대기 중")
        self.progress_var = ctk.StringVar(value="진행 상황: 0/0")
    ...
    # KeywordApp에서 재사용할 유틸리티 함수들 (내부적으로 복사하거나, 상속을 고려할 수 있지만, 일단 복사하여 독립적으로 만듭니다.)
    def _check_log_queue(self):
        """큐에 쌓인 로그 메시지를 메인 스레드에서 처리"""
        try:
            for _ in range(10):
    ...
    def _get_keyword_stats(self, access_key: str, secret_key: str, customer_id: str, hint_keywords: list) -> dict:
        ...
        try:
            self.log_message(f"[Debug BlogAnalyzer] API 요청 URL: {url}")
            self.log_message(f"[Debug BlogAnalyzer] API 요청 Headers: {headers}")
            self.log_message(f"[Debug BlogAnalyzer] API 요청 Params: {params}")
            response = requests.get(url, headers=headers, params=params, timeout=10)
            response.raise_for_status()
            self.log_message("API 응답 성공")
            return response.json()
    ...
    def _create_widgets(self):
        ...
        title = ctk.CTkLabel(container, text="네이버 블로그 포스팅 분석기", font=ctk.CTkFont(size=24, weight="bold"))
        title.pack(pady=(16, 6))
        subtitle = ctk.CTkLabel(container, text="API 정보와 블로그 ID를 입력하고 분석을 시작하세요.", font=ctk.CTkFont(size=14))
        subtitle.pack(pady=(0, 16))
        ...
        # NAVER_BLOG_ID 입력 필드 추가
        blog_id_label = ctk.CTkLabel(api_frame, text="네이버 블로그 ID (rss.blog.naver.com/ID)", anchor="w")
        blog_id_label.grid(row=len(api_fields)+1, column=0, padx=12, pady=6, sticky="w")
        self.blog_id_entry = ctk.CTkEntry(api_frame, placeholder_text="예: myblogid", width=360)
        self.blog_id_entry.grid(row=len(api_fields)+1, column=1, padx=12, pady=6, sticky="ew")
        ...
        self.start_button = ctk.CTkButton(control_frame, text="분석 시작", command=self.start_analysis, height=44, corner_radius=10)
        self.start_button.pack(side="left", padx=12, pady=16, expand=True, fill="x")

        self.stop_button = ctk.CTkButton(control_frame, text="중지", command=self.stop_analysis, height=44, corner_radius=10, fg_color="#8b1a1a", hover_color="#a83232", state="disabled")
        self.stop_button.pack(side="left", padx=12, pady=16, expand=True, fill="x")
        ...
        if os.path.exists(self.config_path):
            try:
                ...
                for key, entry in self.api_entries.items():
                    ...
                    if value := data.get(key, ""):
                        entry.insert(0, value)
                if self.blog_id_entry: # blog_id_entry가 존재할 때만 처리
                    self.blog_id_entry.delete(0, "end")
                    if value := data.get("NAVER_BLOG_ID", ""):
                        self.blog_id_entry.insert(0, value)

                self.log_message("이전에 저장된 API 및 블로그 ID 정보를 불러왔습니다.")
            except Exception as e:
                self.log_message(f"설정 파일을 불러오는 중 오류가 발생했습니다: {e}")

    def _save_config(self, config_data: dict, blog_id: str):
        try:
            config_data["NAVER_BLOG_ID"] = blog_id
            with open(self.config_path, "w", encoding="utf-8") as f:
                json.dump(config_data, f, ensure_ascii=False, indent=2)
            self.log_message("API 및 블로그 ID 정보를 저장했습니다.")
        except Exception as e:
            self.log_message(f"설정 파일 저장 중 오류가 발생했습니다: {e}")
    ...
    def start_analysis(self):
        ...
        self._clear_log()
        self.log_message("블로그 분석 시작 요청을 받았습니다. 설정을 검증합니다...")

        try:
            if self.current_thread and self.current_thread.is_alive():
                print(">>> [DEBUG] Thread already alive")
                self.log_message("이미 분석이 진행 중입니다.")
                messagebox.showinfo("알림", "이미 분석이 진행 중입니다.")
                return

            print(">>> [DEBUG] Reading API config and Blog ID")
            api_config = {key: entry.get().strip() for key, entry in self.api_entries.items()}
            blog_id = self.blog_id_entry.get().strip() if self.blog_id_entry else ""

            missing_fields = [key for key, value in api_config.items() if not value]
            if not blog_id:
                missing_fields.append("NAVER_BLOG_ID")

            if missing_fields:
                print(f">>> [DEBUG] Missing fields: {missing_fields}")
                self.log_message(f"누락된 API 설정 또는 블로그 ID가 있습니다.")
                messagebox.showerror("입력 오류", "모든 API 정보와 블로그 ID를 입력해 주세요.")
                return

            self._save_config(api_config, blog_id)
            self.log_message("설정이 저장되었습니다.")

            self.stop_event.clear()
            self.all_analysis_data = [] # 기존 데이터 초기화
            self.update_progress(0, 1) # 초기 진행바 설정 (총 분석할 글 수를 알 수 없으므로 일단 1로 설정)
            self.status_var.set("블로그 포스팅 분석을 준비하고 있습니다...")

            self.start_button.configure(state="disabled")
            self.stop_button.configure(state="normal")
            self.open_excel_button.configure(state="disabled")

            self.log_message("백그라운드 블로그 분석 작업을 시작합니다.")
            print(">>> [DEBUG] Starting analysis thread")
            self.current_thread = threading.Thread(
                target=self._analyze_blog_posts,
                args=(api_config, blog_id),
                daemon=True,
            )
            self.current_thread.start()
            ...
            print(">>> [DEBUG] Exception in start_analysis:")
            traceback.print_exc()
            self.log_message(f"분석 시작 중 오류 발생: {e}")
            messagebox.showerror("오류", f"작업 시작 중 오류가 발생했습니다: {e}")
            self.start_button.configure(state="normal")
            self.stop_button.configure(state="disabled")
    ...
    def _analyze_blog_posts(self, api_config: dict, blog_id: str):
        # 여기에 블로그 분석 로직 구현
        # 1. RSS 피드 가져오기
        # 2. 각 글 제목에서 키워드 추출
        # 3. 추출된 키워드로 내 블로그 순위 확인
        # 4. 키워드 검색량 조회
        # 5. 판정 결과 생성
        # 6. 결과 저장
        self.log_message("블로그 포스팅 분석 로직을 구현해야 합니다.")
        # 임시 완료 처리
        self.after(0, lambda: self._finish_processing(blog_id))

    def _finish_processing(self, blog_id: str):
        stopped = self.stop_event.is_set()
        self.stop_event.clear()
        ...
        if self.all_analysis_data:
            self.status_var.set("결과를 저장하고 있습니다...")
            self._save_to_excel(blog_id)
            if stopped:
                self.status_var.set("분석이 중지되었으며 수집된 데이터까지 저장했습니다.")
            else:
                self.status_var.set("분석을 완료하고 결과를 저장했습니다.")
        else:
            if stopped:
                self.status_var.set("분석이 중지되었지만 저장할 데이터가 없습니다.")
            else:
                self.status_var.set("수집된 데이터가 없어 파일을 저장하지 않았습니다.")

        self.progress_var.set(f"진행 상황: {len(self.all_analysis_data)}/{len(self.all_analysis_data)}")
        self.progress_bar.set(1 if self.all_analysis_data else 0)

    def _save_to_excel(self, blog_id: str):
        headers = ["제목", "핵심 키워드", "내 순위", "월간 검색량", "판정 결과", "블로그 글 링크"]
        workbook = openpyxl.Workbook()
        sheet = workbook.active
        sheet.title = "블로그 분석 결과"
        sheet.append(headers)

        for row in self.all_analysis_data:
            sheet.append(row)

        safe_blog_id = re.sub(r'[\/*?:"<>|]', "", blog_id)
        excel_file_name = f"블로그_분석_{safe_blog_id}_{time.strftime('%Y%m%d_%H%M%S')}.xlsx"
...
if __name__ == "__main__":
    #app = KeywordApp() # 기존 키워드 채굴기 앱은 주석 처리
    app = BlogAnalyzerApp() # 새로운 블로그 분석기 앱 실행
    app.mainloop()
