        ...
        def callback(result_json):
            try:
                if result_json:
                    ...
                    # Add emoji for trend if > 30
                    formatted = [f"🔥{item['keyword']}" if item.get('trend', 0) > 30 else item['keyword'] for item in data]
                    
                    self.entry_keywords.delete(0, END)
                    self.entry_keywords.insert(0, ", ".join(formatted))
                    self.update_log("✅ 지능형 추천 완료.")
                else:
                    self.update_log("❌ 추천 결과가 없거나 실패했습니다.")
            except Exception as e:
                self.update_log(f"❌ 추천 결과 파싱 오류: {e}")
            finally:
